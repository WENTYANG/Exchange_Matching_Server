...
void reduceMoneyOrSymbol(connection* C,
    ...
    if (amount > 0) {
        ...
        sql << "UPDATE SYMBOL set AMOUNT=SYMBOL.AMOUNT+" << amount
            << ", VERSION = SYMBOL.VERSION+1 WHERE ACCOUNT_ID=" << account_id
            << "AND SYM=" << W.quote(sym) << " AND VERSION=" << version << ";";
    }
...
/*
   ...
   table. Return the results as (state, amount, price, time). There might be 1
   open
...
void cancelOrder(connection* C, int trans_id) {
    ...
    sql << "UPDATE ORDERS SET STATE=" << W.quote("canceled")
        << ", VERSION=ORDERS.VERSION+1 WHERE TRANS_ID=" << trans_id
        << " AND STATE=" << W.quote("open") << ";";
...
/*
    Executed given order. If it is a buy order, the related account will get
   specific amount of symbols. If it is a sell order, the related account will
   specific amount of money. This function does not need to check the version of
   ACCOUNT and SYMBOL table.
*/
void executeOrder(connection* C,
    ...
    if (amount < 0) {  // sell order
        ...
        string sql = "UPDATE ACCOUNT SET BALANCE = ACCOUNT.BALANCE + " +
                     to_string(income) +
        ...
        sql << "INSERT INTO SYMBOL(ACCOUNT_ID, SYM, AMOUNT) VALUES("
            ...
            << ") ON CONFLICT ON CONSTRAINT symbol_pk DO UPDATE SET AMOUNT = "
               "SYMBOL.AMOUNT + "
            << amount << ", VERSION = SYMBOL.VERSION + 1"
            << ";";
        W.exec(sql.str());
    }
    W.commit();
