...
void addOrder(connection * C,
  ...
  if (trans_id != -1) {
    sql << "INSERT INTO ORDERS(TRANS_ID, ACCOUNT_ID, SYM, AMOUNT, "
           "LIMIT_PRICE, STATE, "
           "TIME) VALUES("
        << trans_id << "," << account_id << "," << W.quote(sym) << "," << amount << ","
...
void setOrderExecuted(connection * C,
  ...
  if (rows == 0) {
    throw MyException("Invalid update: version of this order does not match.\n");
  }
  W.commit();
}
...
void cancelOrder(connection * C, int trans_id) {
  ...
  if (rows == 0) {
    ...
    throw MyException("Invalid cancel: Trans_id doesn't exist or the order has "
                      "been fully executed");
  }
...
/*
    Executed given order. If it is a buy order, the related account will get specific amount of symbols.
    If it is a sell order, the related account will specific amount of money. This function does not need to 
    check the version of ACCOUNT and SYMBOL table.
*/
